...
class KyselySearchRepository {
    ...
    ): Promise<Result<Pagination.Paginated<FoodItem.Selectable>, 'ERR_UNEXPECTED'>> {
        try {
            ...
            const totalResult = await this.client
                ...
                .where((eb) => {
                    ...
                    if (options.q) cond = eb.and([cond, eb('name', 'ilike', `%${options.q}%`)])
                    if (options.is_deleted !== undefined) {
                        cond = eb.and([
                            cond,
                            options.is_deleted
...
export default KyselySearchRepository